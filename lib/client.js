...
function ensureEntryStyles() {
  ...
  st.textContent = [
    "#" + CHAT_ENTRY_ID + " { will-change: transform, opacity; }",
    "#" + CHAT_ENTRY_ID + " a { transition: background-color 120ms ease, border-color 120ms ease, width 200ms ease, height 200ms ease, padding 200ms ease; }",
    ...
    // Smoother fade-in/out transitions using CSS transitions
    "#" + CHAT_ENTRY_ID + ".dsh-entry-enter { opacity: 0; transform: translateY(-10px); }",
    "#" + CHAT_ENTRY_ID + ".dsh-entry-enter-active { opacity: 1; transform: translateY(0); transition: opacity 150ms ease-out, transform 150ms ease-out; }",
    "#" + CHAT_ENTRY_ID + ".dsh-entry-exit { opacity: 1; transform: translateY(0); }",
    "#" + CHAT_ENTRY_ID + ".dsh-entry-exit-active { opacity: 0; transform: translateY(-10px); transition: opacity 150ms ease-in, transform 150ms ease-in; }",
    // Rail and wide animations with smoother easing
    "@keyframes dsh-entry-rail-in { from { opacity: 0; transform: translateX(30px); } to { opacity: 1; transform: translateX(0); } }",
    "@keyframes dsh-entry-wide-in { from { opacity: 0; transform: translateY(-5px); } to { opacity: 1; transform: translateY(0); } }"
  ].join("\n");
...
function createEntry() {
  ...
  // beat it.
  let baseBg = "var(--dsw-alias-button-elevated-fill)";
  let hoverBg = "var(--dsw-alias-button-floating-hover)";
...
function updateEntry(host, wide) {
  ...
  if (wide) {
    ...
    a.style.background = "var(--dsw-alias-button-elevated-fill)";
...
function playEntryAnimation(host, wide) {
  ...
  requestAnimationFrame(() => {
    ...
    host.style.animation = wide
      ? "dsh-entry-wide-in 180ms cubic-bezier(0.25, 0.1, 0.25, 1) forwards"
      : "dsh-entry-rail-in 150ms cubic-bezier(0.25, 0.1, 0.25, 1) forwards";
  });
}
function mountBelowEntry(wide) {
  ensureEntryStyles();
  // Cancel any pending exit if re-mounting during exit animation
  ...
  if (!host) {
    ...
    host.classList.add('dsh-entry-enter');
    nb.parentElement.insertBefore(host, nb.nextSibling);
    
    // Trigger enter animation
    requestAnimationFrame(() => {
      void host.offsetHeight; // Force reflow
      host.classList.add('dsh-entry-enter-active');
      
      // Clean up classes after animation
      setTimeout(() => {
        host.classList.remove('dsh-entry-enter', 'dsh-entry-enter-active');
      }, 160);
    });
  }
...
function unmountBelowEntry() {
  ...
  // Add exit animation
  host.classList.remove('dsh-entry-enter', 'dsh-entry-enter-active');
  host.classList.add('dsh-entry-exit');
  
  // Force reflow to apply exit class
  void host.offsetHeight;
  host.classList.add('dsh-entry-exit-active');
...
function ChatEntry({ wide, useShow }) {
  ...
  const obsRef = (0, import_react.useRef)(null);
  ...
  (0, import_react.useEffect)(() => {
    if (!show) {
      // When hiding, cleanup and unmount
      cleanup();
    ...
    const tryMount = () => {
      if (!mountedRef.current && mountBelowEntry(wide)) {
    ...
    // If not mounted yet, set up polling and observer
    if (!mountedRef.current) {
      timerRef.current = setInterval(tryMount, 100); // Reduced interval for faster response
      obsRef.current = new MutationObserver(tryMount);
