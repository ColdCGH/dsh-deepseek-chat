...
function playRailInAfterCollapse(host) {
  // Hide while the sidebar shrinks so the entry is NOT pulled down with the
  // other icons during the collapse.
  host.style.transition = "none";
  host.style.animation = "none";
  host.style.opacity = "0";
  const ref = findNewSessionButton();
  _railWait.cancel = false;
  let prev = ref ? ref.getBoundingClientRect().width : -1;
  let stable = 0;
  let frames = 0;
  const tick = () => {
    if (_railWait.cancel) { _railWait.raf = 0; return; }
    frames++;
    const w = ref ? ref.getBoundingClientRect().width : -1;
    if (w === prev) stable++; else stable = 0;
    prev = w;
    // Collapse is done once the New Session button stops moving (3 stable
    // frames); the cap guarantees the entry can never be stuck hidden.
