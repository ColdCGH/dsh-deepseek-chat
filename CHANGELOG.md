...
- 收起同步滑入（0.2.7）、rail hover 背景（0.2.8）、收起/展开无抖动闪现（0.2.9）保持不变

## 0.2.9 (2026-08-19)

- **Fix（真实 harness 实测）**：
  1. **收起时图标下方不再轻微上移抖动**：锚点原带 `width/height/padding 200ms` 过渡，收起时入口 38→36 缓慢缩小 200ms，导致下方工作区/会话图标被 reflow 顶得轻微上移。去掉锚点尺寸过渡 → 尺寸在 rail snap 时一次到位，随整个侧边栏一次定位
  2. **展开时不再闪现**：原宽高尺寸各自渐变（宽度先 snap、高度再 36→38 过渡），与 wide-in 淡入时序不一致 → 「闪现」。统一尺寸即时 snap + 200ms 平滑淡入
- 收起同步滑入（0.2.7）、rail hover 背景修复（0.2.8）保持不变
